...
**One spec item with no task, by design:** `missing_priority_section`. mitodo's `store::create_item` places by section and falls back to appending at end of file rather than refusing, so there is no code path that can raise it. Rather than invent a check, Task 5 keeps `invalid_group` and `validation_error`; the code is listed in the spec's table but never emitted. Flagged here so the discrepancy is deliberate rather than forgotten.
